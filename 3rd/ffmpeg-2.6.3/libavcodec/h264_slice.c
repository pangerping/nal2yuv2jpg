...
#if 0
    // add by yujun
    if (h->slice_type == AV_PICTURE_TYPE_I && h->avctx->error_frame < 0) {
        if (h->avctx->last_error_frame < 0) {
            h->avctx->last_error_frame++;
        } else {
            h->avctx->error_frame = 0;
        }
    }
    // add end
#endif
...
#if 0
    //add by yujun
    if (h->avctx->error_frame == -1) {
        av_log(h->avctx, AV_LOG_ERROR,
                "I frame is error_frame = -1\n");
        return AVERROR_INVALIDDATA;
    }
#endif
    return 0;
